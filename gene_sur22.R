gene_sur22 <- function(genelist, genelist2 ) {
...
a <- data.frame(vst@assays@data@listData[[1]][geneids,])
 ...
 geneids <- geneidnames[geneidnames$gene_name %in% genelist2, ]$gene_id
geneids <- geneids[geneids %in% rownames(vst@assays@data@listData[[1]])]
...
a <- cbind(a1*length(genelist)/(length(genelist) + length(genelist2)), -a * length(genelist2)/(length(genelist) + length(genelist2)))
...
rownames(sinfo) <- str_replace(rownames(sinfo), "-", ".")
#rownames(sinfo) <- str_replace(rownames(sinfo), "_", ".")
